...
int main(int argc, char **argv) {
  ...
  // {
  //   uint32_t inv[] = {1, 0, 3, 0, 0, 0, 2};
  //   uint32_t* temp;
  //   checkCudaErrors(cudaMalloc(&temp, sizeof(uint32_t) * 1024));
  //   checkCudaErrors(cudaMemset(temp, 0, sizeof(uint32_t) * 1024));
  //   checkCudaErrors(cudaMemcpy(temp, inv,
  //                              sizeof(inv), cudaMemcpyHostToDevice));
  //   your_sort(temp, inputPos, outputVals, outputPos, sizeof(inv) / sizeof(inv[0]));
  // }
  timer.Stop();
